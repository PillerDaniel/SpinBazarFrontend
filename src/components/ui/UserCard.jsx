...
import { useUser } from "../../context/UserContext"; // Feltételezve, hogy ez létezik
              ...
              <form onSubmit={(e) => e.preventDefault()} className="space-y-4">
                ...
                {!showCardDetails && (
                   <button
                     ...
                     // VÁLTOZTATÁS: hover:bg-right helyett hover:opacity-90
      ...
      {showCardDetails && activeTab === 'deposit' && (
        ...
        >
          <form onSubmit={handleDeposit} className="p-6 space-y-5">
            ...
            {/* Kártyaszám */}
            <div>
                <label htmlFor="cardNumber_details" className="block text-xs font-medium mb-1 text-gray-400 uppercase tracking-wider">{t("card_number_label")}</label> {/* Added unique ID */}
                <div className="relative flex items-center bg-gray-800 border border-gray-700 rounded-md px-3 py-2 focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-blue-500">
                  <input
                    ...
                    className="flex-grow bg-transparent border-none text-base sm:text-lg font-mono text-white focus:outline-none placeholder-gray-500"
                    placeholder="---- ---- ---- ----" value={formatCardNumber(cardNumber)}
                    onChange={(e) => setCardNumber(e.target.value)}
                    maxLength="19" required
                  />
                  <span className="text-gray-500 font-bold text-sm ml-2">VISA</span>
                </div>
            </div>

            {/* Kártyabirtokos neve */}
            <div>
                <label htmlFor="cardHolderName_details" className="block text-xs font-medium mb-1 text-gray-400 uppercase tracking-wider">{t("cardholder_name_label")}</label> {/* Added unique ID */}
                <input
            ...
            <div className="flex space-x-4">
              <div className="w-1/2">
                <label htmlFor="cardExpiry_details" className="block text-xs font-medium mb-1 text-gray-400 uppercase tracking-wider">{t("expiry_date_label")}</label> {/* Added unique ID */}
                <input
              ...
              <div className="w-1/2">
                <label htmlFor="cardCVV_details" className="block text-xs font-medium mb-1 text-gray-400 uppercase tracking-wider">{t("cvv_label")}</label> {/* Added unique ID */}
                <input
